...
def train_one_epoch(
    model, criterion, train_dataloader, optimizer, epoch, clip_max_norm, logger_train, tb_logger, current_step
):
    model.train()
...
def main(argv):
    ...
    image_util.setup_logger('train', os.path.join('../experiments', args.experiment), 'train_' + args.experiment, level=logging.INFO,
                        screen=True, tofile=True)
    image_util.setup_logger('val', os.path.join('../experiments', args.experiment), 'val_' + args.experiment, level=logging.INFO,
                        screen=True, tofile=True)
    ...
    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[100,125], gamma=0.1)
